...
Function Submit-LogAnalyticsData($customerId, $sharedKey, $body, $logType)
{
    ...
    Write-Host "$TimeStampField Submit log entry"
...
$logType = "{0}AdoPipelineType" -f $Organization

# Iterate through all projects to get repos
Write-Host "Iterate through all projects to get pipelines"
$projectsResult.value | ForEach-Object {
    $projectId = $_.id
    $pipelineUri = "https://{0}/{1}/_apis/pipelines?api-version={2}" -f $coreServer, $projectId, $apiVersion
    $pipelineResult = Get-RestCallWithContinuationTokenResult -Uri $pipelineUri -Header $header -ErrorAction Stop
    $jsonValue = $pipelineResult.value | ConvertTo-Json -Depth 10
    ...
    if ($null -ne $jsonValue) {
        ...
        Write-Host "Skipping log entry for project $projectId"
    }
}
